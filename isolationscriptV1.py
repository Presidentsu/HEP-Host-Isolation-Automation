...
def run_search_and_isolate(config: Config) -> bool:
    ...
    if not infinity_api.authenticate():
        logger.error("Failed to authenticate with Infinity Events")
        return False
    
    if not hep_api.authenticate():
        logger.error("Failed to authenticate with Harmony Endpoint")
        return False
    
    # Prepare timeframe
    if config.automation_mode and config.time_window_hours:
        now = datetime.datetime.utcnow()
        start_time = now - datetime.timedelta(hours=config.time_window_hours)
        timeframe = {
            "startTime": start_time.strftime("%Y-%m-%dT%H:%M:%SZ"),
            "endTime": now.strftime("%Y-%m-%dT%H:%M:%SZ")
        }
    ...
    for attempt in range(max_attempts):
        ...
        if status['state'] == 'Ready':
            ...
            logger.error(f"Search failed: {status.get('errors', [])}")
            return False
        ...
        logger.error("Search timed out after 5 minutes")
        return False
    ...
    logger.info(f"Found {len(all_machines)} affected machines")
    
    # Handle approval/automation
    selected_machines = []
    if config.approval_required:
        selected_machines = wait_for_approval(all_machines)
        if not selected_machines:
            logger.info("User chose to skip isolation")
            return True
    else:
        # Auto mode - get unique machine names
    ...
    if selected_machines:
        ...
        if success:
            ...
            logger.error(f"Failed to initiate isolation: {result}")
        return success
